...
public class AutofillPopup extends Window {
	...
	public AutofillPopup(final String efoTerm,
			final EFOServiceAsync efoServiceAsync, final GuiMediator guiMediator) {
		super();
		...
		final AsyncCallback<String> callback2 = new AsyncCallback<String>() {
			...
			public void onSuccess(String description) {
				efo_description.setContents("Description:<br>"
		...
		characteristicInput
				.setValueMap(GlobalConfigs.getCommonFactorsOrChars());
		...
		form.setFields(addToAllRecordsCheckBox, newCharacteristicCheckbox,
				characteristicInput, termSourceRefCheckbox, termSourceRef,
				termSourceNumberCheckbox, termSourceNum);
	...
	private void submit(String efoTerm) {
		...
		boolean termSourceRefChecked = termSourceRefCheckbox
				.getValueAsBoolean();
		boolean termSourceNumberChecked = termSourceNumberCheckbox
				.getValueAsBoolean();
		boolean newCharacteristicChecked = newCharacteristicCheckbox
				.getValueAsBoolean();
		...
		if (addToAllRecordsCheckBox.getValueAsBoolean() == true) {
			if (termSourceNumberChecked) {
				guiMediator.addColumnToCharacteristicAndAddValueToAllRecords(
						"Term Accession Number", termSourceNum
								.getDisplayValue());
			}
			...
			if (newCharacteristicChecked) {
				guiMediator.addColumnToCharacteristicAndAddValueToAllRecords(
						"Characteristics[" + characteristicString + "]",
						efoTerm);
			}
		}
