...
public class SDRF_Section_ColumnEditor extends Window {
	 ...
	 * 
	 * @param guiMediator
	 *            the mediator, which allows for this window to update other
	 *            windows.
	 */
	...
	private void saveAction() {
		...
		for (int i = 0; i < activeFieldArray.length; i++) {
			newActiveColumns[i + 1] = new ListGridField(activeFieldArray[i]
					.getAttribute("key"), activeFieldArray[i]
					.getAttribute("title"));
			if (GlobalConfigs.shouldExclude(activeFieldArray[i]
					.getAttribute("title"))) {
	...
	/**
	 * Gets an RecordList, which is basically an array of fields. This retrieves
	 * the list from the SDRF Table. The order of the records determines the
	 * order of the columns.
	 * 
	 * For each listGridRecord, there is a key and title attribute. The key
	 * attribute uniquely identifies the column and is used to retrieve the data
	 * 
	 ...
	 * (Key=3)...(Title= Sample)
	 * 
	 * @return an array of ListGridRecords, which describes the order of the
	 *         columns
	 */
	private RecordList convertFieldsToRecords() {
		...
		for (int i = 0; i < fields.length; i++) {
			if (i != 0) {
				...
				record.setAttribute("title", fields[i].getTitle());
				record.setAttribute("key", fields[i].getName());
