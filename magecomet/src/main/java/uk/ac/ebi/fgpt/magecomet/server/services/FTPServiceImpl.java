...
public class FTPServiceImpl extends RemoteServiceServlet implements FTPService {
	...
	public String getExperimentJSON(String experimentAccession)
			throws FTPException {
		...
		try {
			...
			File sdrfFile = hashOfAccessionFilesForSDRF
			...
			responseJSONObject.put("sdrfArray", JSONUtils
					.getJSONArrayFromSDRF(sdrf));
			...
			responseJSONObject.put("idfArray", JSONUtils
					.getJSONArrayFromIDF(idf));
			...
			// Error Items
			responseJSONObject.put("error", JSONUtils.getErrorArray(
					hashOfAccessionFilesForIDF.get(experimentAccession),
	...
	private void downloadAccession(String accession) throws FTPException {
		...
		try {
			...
			try {
				...
				client.retrieveFile(arrayExpressFtpPath + pipeline + "/"
						+ accession + "/" + sdrfFile, fos_sdrf);
